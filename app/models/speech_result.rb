class SpeechResult < ApplicationRecord
  ...
  def get_keywords(alchemy_response)
    alchemy_response["keywords"].map do |keyword|
      ...
      keyword_emotion = KeywordEmotion.create(
        anger: keyword["emotions"]["anger"],
        disgust: keyword["emotions"]["disgust"],
        fear: keyword["emotions"]["fear"],
        joy: keyword["emotions"]["joy"],
        sadness: keyword["emotions"]["sadness"]
      )
  ...
  def personality_profile
    tone_scores = get_personality_scores
    traits = collect_traits(tone_scores)
    if traits.empty?
  ...
  private
    ...
    def get_personality_scores
      Hash[self.doc_social_tone.as_json.collect {|tone, score|
        if score > 0.85
          [tone, "high"]
        elsif score < 0.4
          [tone, "low"]
    ...
    def collect_traits(tone_scores)
      traits = []
